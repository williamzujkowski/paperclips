...
const ACHIEVEMENTS = {
  ...
  wireEfficiency: {
    ...
    progress: (state) =>
      Math.min(state.achievements.maxClipsPerSpool || 0, 1000),
    maxProgress: 1000,
  },
  ...
  speedRun1Hour: {
    ...
    icon: "⏱️",
    condition: (state) =>
      state.resources.totalClips >= 1000000 &&
      state.achievements.gameTime < 3600,
  ...
  speedRun30Min: {
    ...
    icon: "🏃",
    condition: (state) =>
      state.resources.totalClips >= 100000 &&
      state.achievements.gameTime < 1800,
  ...
  tenProjects: {
    ...
    progress: (state) =>
      Math.min(state.achievements.projectsCompleted || 0, 10),
    maxProgress: 10,
  },
  ...
  noAutoclippers: {
    ...
    icon: "✋",
    condition: (state) =>
      state.resources.totalClips >= 10000 &&
      state.production.autoClippers === 0,
    progress: (state) =>
      state.production.autoClippers === 0
        ? Math.min(state.resources.totalClips, 10000)
        : 0,
    maxProgress: 10000,
    hidden: true,
  },
...
export class AchievementSystem {
  ...
  notifyListeners(achievement) {
    this.listeners.forEach((listener) => {
      try {
        ...
        errorHandler.log(
          "error",
          "Error notifying achievement listener",
          error,
        );
      }
    });
  ...
  getProgress(achievementId) {
    ...
    const progress =
      gameState.get(`achievements.progress.${achievementId}`) || 0;
  ...
  getRarityStats() {
    ...
    for (const rarity of Object.values(AchievementRarity)) {
      const achievements = Object.values(this.achievements).filter(
        (a) => a.rarity === rarity,
      );
      const unlockedCount = achievements.filter((a) => unlocked[a.id]).length;
